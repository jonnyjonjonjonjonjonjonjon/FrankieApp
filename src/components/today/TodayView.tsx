...
function BigTile({ event, dayWord, onClick }: { event: DiaryEvent | null; dayWord?: string; onClick: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex min-h-0 flex-1 items-center gap-4 rounded-3xl border-4 border-ink bg-paper px-4 py-2 text-left active:scale-[0.98] sm:gap-5 sm:px-5 sm:py-3"
    >
      ...
      <div className="flex min-w-0 flex-col gap-1">
        {dayWord && <span className="text-lg font-bold text-ink-soft sm:text-2xl">{dayWord}</span>}
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden px-3 py-2 sm:py-3">
        <div className="mx-auto flex min-h-0 w-full max-w-4xl flex-1 flex-col gap-2 sm:gap-3">
          <Clock />
