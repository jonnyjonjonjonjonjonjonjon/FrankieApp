...
| 4.2 Week table Mon–Sun: staying-at, breakfast/lunch/dinner with food, events, photo previews; birthdays band; the day/date row stays at the top when scrolling | `components/week/WeekView.tsx` |
| 4.3 Month grid, today in orange, per-day markers (staying away, doctor/dentist, visitors, birthdays); centred title with arrows either side; months slide side to side like days | `components/month/` |
...
| 4.6 "+ Add" everywhere; add-to-day with no typing; new word = Word → Picture (symbol grid with search and/or photo) → Yes; question screens answer with thumbs-up Yes / thumbs-down No (and Clear to take a value away) | `components/day/AddEventFlow.tsx`, `components/pickers/NewItemForm.tsx` |
